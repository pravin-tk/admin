...
%>
                        ...
                        <div class="tab-pane fade active in" id="contacts" aria-labelledby="contacts-tab">
                            ...
                            <div class="contacts-list">
                             <h2>List Safety Category Item</h2>
                                <p>Here you can add or deactivate category item. You can define category item details, .</p>
                            <div class="form-group">
                                ...
                                <a href="#" class="btn btn-primary view-contacts bottom-margin"><i class="fa fa-plus"></i> Category Item</a>
                                    ...
                                    <tbody>
                                    	...
                                    	if(item_size>0)
	                                        ...
	                                        <tr>
	                                            ...
	                                            <td><% if(item_list.get(i).getStatus() == 1) { out.print("<span class='label label-success'>Active</span>"); } else { out.print("<span class='label label-warning'>Inactive</span>"); } %></td>
	                                             <td><% out.print(item_list.get(i).getDescription()); %></td>
	                                            <td class="alignRight">
	                                            	<a href="javascript:editCatItem(<% out.print(item_list.get(i).getId()); %>);" class="btn btn-success icon-btn"><i class="fa fa-pencil"></i></a>
	                                            </td>
	                                        </tr>
	                                        <% } %>
                                    </tbody>
                                ...
                                <a href="#" class="btn btn-primary view-contacts"><i class="fa fa-plus"></i> Category Item</a>
 ...
 <div id="aerror" class="bg-danger" ></div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-2">
                                        <input type="text" class="form-control" name="tname" id="tname" placeholder="eg. Identity Card, Lab Coat">
                                    </div>
                                    <div class="col-sm-8">
                                        <div class="tooltip custom-tool-tip right">
                                            <div class="tooltip-arrow"></div>
                                            <div class="tooltip-inner">
                                                This is the name of the safety category item.
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    <div class="col-sm-2">
                                        <button type="button" class="btn btn-success" onclick="saveCatItem();">Save</button>
...
function saveCatItem(){
	...
	if (strcat == "" ) {
		...
		$.post("../webapi/facility/safetycatitem/save", {name: $("#tname").val(),
		categoryId: $("#catId").val(), desc: $("#desc").val(), 
		status: $('input[name=status]:checked').val()}, function(data){
		if($("#catId").val() > 0){
			if(data.status == 1)
				window.location.href = "${baseUrl}/facility/safety-cat-item.jsp?scat_id="+$("#catId").val();
			else
				alert(data.message);
		}else{
...
function editCatItem(id){
	window.location.href = "${baseUrl}/facility/editsafety-cat-item.jsp?scatitem_id="+id;
}

$("#searchcatId").change(function(){
	if($(this).val() > 0){
		window.location.href = "${baseUrl}/facility/safety-cat-item.jsp?scat_id="+$(this).val();
	} else {
		alert("select Category");
	}
});
